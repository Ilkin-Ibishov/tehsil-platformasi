...
- **PowerShell `curl` Alias Trap**: In Windows PowerShell, `curl` is an alias for `Invoke-WebRequest`. Running `curl -I <url>` invokes PowerShell's cmdlet instead of the native binary, leading to hangs or syntax discrepancies in non-interactive tasks. Always execute `curl.exe -I <url>` explicitly when inspecting HTTP status and headers.
